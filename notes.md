...
## [2026-07-05T04:00:00Z] P4/P5/P6 -- RQ3 track COMPLETE (oos-main-v2)
- oos-main-v1 (chunk 512) FAILED: 2/3 jobs hung >1h in KV preemption thrash (512 prompts > ~156 concurrency at max_model_len 2048). Cancelled. FIX (commit d4684a0): run_offline chunk 128 (< concurrency limit) -> 0 preemption warnings, stable. worker:v17.
- oos-main-v2 (worker:v17, chunk 128, 3 models PARALLEL, alias): 10 tickers x 15 agents/model x 30 days = 13500 decisions, valid_rate 0.9994, 300 ticker-days. All 3 SUCCEEDED cleanly (Qwen1.5B straggled but finished; no stall). Collected -> signals.jsonl (gcloud-CLI collector, commit 0c39219/bce3b68 fixed local ADC 403 + Windows gcloud.cmd).
- P5 stats (scripts/p5_stats.py, evals/prediction.py, n_trials=4): docs/P5_STATS.md.
  - imbalance_cw/uw 1d: IC ~+0.017, CI includes 0, hit <0.5 -> NO next-day predictability.
  - imbalance_cw/uw 5d: IC ~-0.137/-0.135, 95% CI EXCLUDES 0 ([-0.271,-0.007]) -> small statistically-detectable CONTRARIAN tilt (crowd buys -> lower 5d return). Reflects the crowd's strong optimism bias (mean imbalance ~+0.5).
  - Diebold-Mariano vs 5-day momentum: NOT significant (p>=0.26) at any horizon -> no incremental info over momentum.
