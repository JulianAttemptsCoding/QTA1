...
def run_offline(requests_path: Path, out_path: Path, model_id: str,
                max_new_tokens: int = 160, chunk: int = 128,
    ...
    # no bf16, FlashAttention-2 unsupported). max_model_len 2048 fits the trimmed ~600-700-tok
    # prompt (20 bars / 3 news, see sim_prompts) + the 160-tok JSON. Raw completion strings.
    # chunk=128: the T4 KV cache holds ~156 concurrent 2048-token sequences, so feeding >156
    # prompts per generate() call over-subscribes it and vLLM thrashes in PreemptionMode.
    # RECOMPUTE (this hung two chunk-512 jobs in oos-main-v1 for >1h). Keeping the batch under
    # the concurrency limit means no preemption -> stable and, net of recompute waste, faster.
    # It also bounds host RAM (the earlier n1-standard-8 OOM); swap_space capped at 2 GiB.
